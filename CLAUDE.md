...
| Database | PostgreSQL 18 (Docker) | dev: `localhost:5434` |
...
- PostgreSQL 18 รันใน Docker container `stockapp-postgres` (map `5434:5432`)
- `DATABASE_URL` อยู่ใน `.env` (gitignored) — template สำหรับทีมอยู่ที่ `.env.example`
- dev credentials: `stockuser` / `stockpass` / db `stockdb` @ `localhost:5434`
